            ...
            // console.log("Status code: " + body);
                if(response.statusCode === 200) {
                    ...
                    var analysis_object = {};
                        ...
                        //to parse the object into desired one
                        
                        var total = 0;
                        var final_object = {};
                        for(let key in analysis_object){
                            console.log()
                            total = total + parseInt(analysis_object[key].count);
                        }
                        for(let key in analysis_object){
                            final_object[key] = parseInt(analysis_object[key].count)/total;
                        }
                        ...
                        console.log("<==============  NORMLISED  ===============>");
                        console.log(final_object);

                        console.log("TOTAL KEYS FETCHED : "+ keysin.length);

                        console.log("<==============  PARSED ARRAY  ===============>");
                        var parsed_array = parseObject(final_object);
                        console.log(parsed_array);
                        console.log("PARSED ARRAY LENGTH => "+parsed_array.length);
...
function parseObject(final_object){
    var ref_array = ['#','%','(',')','+',',','/','[',']','addEventListener','attachEvent','char_count_in_identifier','concat','console','cookie','createElement','digit_count_in_identifier','divided_url','document','dot','escape','eval','function','getElementById','hexa_count','iframe_count','indexOf','location','log','onerror','onload','parseInt','random','replace','setAttribute','setTimeout','space_count_in_raw','split','substring','unescape','var','window','write','{','|','}'];
    console.log(ref_array.length);
    var ans_array = [];
    ref_array.forEach(function(item,index){
        if(item in final_object){
            ans_array.push(final_object[item]);
        }else{
            ans_array.push(0);
        }
